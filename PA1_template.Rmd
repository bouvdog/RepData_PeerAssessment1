...
`r stepsPerDay<-activityByDay[activityByDay$steps > 0,]`
**Mean steps per day:** `r as.integer(mean(stepsPerDay$steps))`  
**Median steps per day:** `r median(stepsPerDay$steps)`
...
**Average Max Steps Interval: **`r avgStepsInInterval[avgStepsInInterval$x == max(avgStepsInInterval$x), ]$interval`
...
```  
**Number of rows with 'NA's:** `r totalNumberOfRows - numberOfRowsWithoutNAs`
...
`r stepsPerDay<-activityByDayWithValuesForNA[activityByDayWithValuesForNA$steps > 0,]`
  
**Mean number of total steps taken per day:** `r as.integer(mean(stepsPerDay$steps))`  
**Median number of total steps taken per day:** `r as.integer(median(stepsPerDay$steps))`
